...
from fgsm_yolov8 import YoloV8FGSM
...
FGSM_ENABLE = os.environ.get("FGSM_ENABLE", "0") == "1"
FGSM_EPSILON = float(os.environ.get("FGSM_EPSILON", str(4.0 / 255.0)))
FGSM_BOX_GAIN = float(os.environ.get("FGSM_BOX_GAIN", "7.5"))
FGSM_CLS_GAIN = float(os.environ.get("FGSM_CLS_GAIN", "0.5"))
FGSM_DFL_GAIN = float(os.environ.get("FGSM_DFL_GAIN", "1.5"))
FGSM_PSEUDO_CONF = float(os.environ.get("FGSM_PSEUDO_CONF", "0.25"))
FGSM_PSEUDO_MAX_DET = int(os.environ.get("FGSM_PSEUDO_MAX_DET", "30"))
...
if FGSM_ENABLE:
    try:
        _fgsm = YoloV8FGSM(
            ...
            box_gain=FGSM_BOX_GAIN,
            cls_gain=FGSM_CLS_GAIN,
            dfl_gain=FGSM_DFL_GAIN,
            pseudo_conf=FGSM_PSEUDO_CONF,
            pseudo_max_det=FGSM_PSEUDO_MAX_DET,
        )
        _log(
            f"[fgsm] enabled eps={FGSM_EPSILON:.6f} "
            f"gains(box={FGSM_BOX_GAIN}, cls={FGSM_CLS_GAIN}, dfl={FGSM_DFL_GAIN})"
        )
    except Exception as exc:
        _fgsm = None
        _log(f"[fgsm] disabled due to init error: {exc}")
...
def apply_fgsm_to_yolo_input(frame_bgr):
    """Return 640x640 YOLO input image, optionally FGSM-perturbed."""
    ...
    try:
        adv_img, stats = _fgsm.generate(img)
        _log(
            f"[fgsm] loss(total={stats['total']:.4f}, box={stats['box']:.4f}, "
            f"cls={stats['cls']:.4f}, dfl={stats['dfl']:.4f}, targets={stats['targets']})"
        )
        return adv_img
    except Exception as exc:
        _log(f"[fgsm] generate failed, using clean image: {exc}")
        return img
...
# ============================ INFERENCE =====================================
@torch.no_grad()
def infer(frame_bgr, context=None, yolo_input_bgr=None):
    """
    Returns:
    ...
    if frame_bgr.shape[:2] == (640, 640):
        ...
        img = cv2.resize(frame_bgr, (640, 640))
    if yolo_input_bgr is None:
        yolo_img = img
    elif yolo_input_bgr.shape[:2] == (640, 640):
        yolo_img = yolo_input_bgr
    else:
        yolo_img = cv2.resize(yolo_input_bgr, (640, 640))
    ...
    # YOLO detection
    dets = _yolo.predict(yolo_img, conf=0.25, imgsz=320, verbose=False)[0]
